...
df_DSB = pd.read_csv(path + "/Water_Temp.csv", sep=",", header=0, parse_dates=["Datetime"])
...
df_kit = df_kit[mask]
path_AWS = "./data_AWS"
df_AWS = pd.read_csv(path_AWS + "/HIAL_Table15min.dat", skiprows = 1, header=0, parse_dates=["TIMESTAMP"])
df_AWS = df_AWS[2:]
...
df_AWS2 = pd.read_csv(path_AWS + "/HIAL_Table60Min.dat", skiprows = 1, header=0, parse_dates=["TIMESTAMP"])
df_AWS2 = df_AWS2[2:]
...
RMSE_T = (((df_out.T_AWS - df_out.T_Pi) ** 2).mean() ** .5)
RMSE_T2 = (((df_out.T_AWS - df_out.Water_Temp) ** 2).mean() ** .5)
RMSE_v= (((df_out.v_AWS - df_out.v_Pi) ** 2).mean() ** .5)
RMSE_RH = (((df_out.RH_AWS- df_out.RH_Pi) ** 2).mean() ** .5)
RMSE_P= (((df_out.P_AWS- df_out.P_Pi) ** 2).mean() ** .5)
...
ax1.grid()
lims = [
np.min([-25,-25]),  # min of both axes
np.max([25, 25]),  # max of both axes
]
# now plot both limits against eachother
ax1.plot(lims, lims, '--k', alpha=0.25, zorder=0)
ax1.set_aspect('equal')
